...
def get_subsectors(sector_map: pd.DataFrame, ccc_sector: str): #Joris - This makes sense.
    """
...
def load_nzip(nzip_path: str, sector_map_path: str, sector: str, scenario): #Joris - This makes sense.
    """
...
def add_cols(df): #Joris - This makes sense.
    """
    ...
    #df.to_excel("Years problem.xlsx")
    ...
    for y in YEARS:
        ...
        df[f'capex annualised {y}'] = df[f'AM levelised capex (£m) {y}'] - df[f'Counterfactual levelised capex (£m) {y}']
        ...
        df.loc[y < df['Year of Implementation'], f'capex low carbon {y}'] = 0 # unflip signs
        #df[f'capex low carbon {y}'] = df.apply(lambda row: 0 if row['Year of Implementation'] < y else row[f'AM capex (£m) {y}'], axis=1)
...
def add_reee(nzip_path, df, scenario, baseline_col, post_reee_col, out_col, usecols="E:AL", header=327, nrows=28):
    df = df.copy()
...
def sd_measure_level(df, args, scenario, reee_args=None, baseline=True, nzip_path=None):
    ...
    if not baseline:
        ...
        for kwargs in reee_args:
            ...
            reee_df = add_reee(nzip_path, df, scenario, **kwargs)
...
#def get_additional_demand_agg(df, agg_df, scenario, fuel, fuel_out=None, tech='CCS'):
    """
...
def get_additional_demand_agg(df, agg_df, scenario, fuel, fuel_out=None, tech='CCS'):
    ...
    # Filter the DataFrame for the given technology type
    ccs_df = df.loc[df['Technology Type'] == tech].copy()
    year_of_implementation = ccs_df['Year of Implementation']
    ...
    for region in regions:
        ...
        regional_df = ccs_df.loc[ccs_df['Country'] == region]

        # Perform the calculations for the current region
        for y in YEARS:
            regional_df[f'{fuel} use after implementation {y}'] = regional_df[f'Total {fuel} use (GWh) {y}'].copy()
            regional_df.loc[y < year_of_implementation, f'{fuel} use after implementation {y}'] = 0

            # Calculate the sum for the year and convert from GWh to TWh
            agg_df.loc[f'Additional demand {fuel_out} abated ({region})', y] = regional_df[f'{fuel} use after implementation {y}'].sum() * 0.001

        # Update the aggregate DataFrame for each region
        agg_df.loc[f'Additional demand {fuel_out} abated ({region})', 'Aggregate Variable'] = f'Additional demand {fuel_out} abated ({region})'
        agg_df.loc[f'Additional demand {fuel_out} abated ({region})', 'Variable Unit'] = 'TWh'
        agg_df.loc[f'Additional demand {fuel_out} abated ({region})', 'Scenario'] = scenario

    return agg_df


def get_aggregate_df(df, measure_level_kwargs, scenario, baseline_kwargs, sector):
    """
    ...
    # get total emissions
    sd_df = sd_measure_level(df, measure_level_kwargs, scenario)
    #sd_df = sd_measure_level(df.copy(), measure_level_kwargs, scenario, reee_args, baseline=False)
    #sd_df.to_excel('ABC.xlsx')
    bl_df = sd_measure_level(df, baseline_kwargs, scenario)
    bl_df = baseline_from_measure_level(bl_df)    
    
    # get traded emissions
    df_traded = df.loc[df['Traded / non-traded'] == 'traded'].copy()
    sd_df_traded = sd_measure_level(df_traded, measure_level_kwargs, scenario)
    #sd_df = sd_measure_level(df_traded.copy(), measure_level_kwargs, scenario, reee_args, baseline=False)
    bl_df_traded = sd_measure_level(df_traded, baseline_kwargs, scenario)
    ...
    (bl_df['Baseline Variable'].isin(['Baseline emissions CO2', 'Baseline emissions CH4', 'Baseline emissions N20'])) & 
    (bl_df['Country'] == 'United Kingdom')
    ...
    (bl_df_traded['Baseline Variable'].isin(['Baseline emissions CO2', 'Baseline emissions CH4', 'Baseline emissions N20'])) & 
    (bl_df_traded['Country'] == 'United Kingdom')
    ].sum(numeric_only=True)
    ...
    agg_df = get_additional_demand_agg(df, agg_df, scenario,'natural gas', 'gas')
    #agg_df = get_additional_demand_agg(df, agg_df, scenario,'petroleum')
    #agg_df = get_additional_demand_agg(df, agg_df, scenario,'solid fuel')
